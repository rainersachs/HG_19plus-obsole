...
source("p_monteCarlo.R") # Load Monte Carlo.
...
#=============== Fig. for visual check of models ===================#
dvchk <- 0.5*0:200 
plot(c(0, 100), c(-.02, 1), pch = 19, col = 'white', ann = FALSE, bty = 'u')
#aa1=9.889e-05; aa2 = 4.057e-03; kk1 = 5.321e-02
LET = 253
lines(dvchk, .0275 + (1 - exp ( - (aa1 * LET * dvchk * exp( - aa2 * LET) 
                                   + (1 - exp( - phi * dvchk)) * kk1))), lwd = 2)#NTE 3 param  HZE model modifies the hazard function NTE models in 17Cuc. 

# modified_df %>% filter(Z < 3 & d>0)# 2 lines show grammar
#loLET_df = modified_df %>% filter(Z < 3 & d>0)

visual_data <- ion_data %>% filter (LET == 253)#needs number
errbar(visual_data[, "dose"], visual_data[, "Prev"], 
       yplus  = visual_data[, "Prev"] +  visual_data[, "SD"],
       yminus = visual_data[, "Prev"] -  visual_data[, "SD"],
       pch = 19, cap = 0.02, add = TRUE, col = 'red', errbar.col = 'red', lwd = 2)
legend(x = "topleft", legend = "LET = 253",cex=0.5)                               
#========================================================#
